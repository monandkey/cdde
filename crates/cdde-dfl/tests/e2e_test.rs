...
async fn test_e2e_flow() {
    ...
    let mut client = cdde_proto::core_router_service_client::CoreRouterServiceClient::connect(format!("http://{}", dcr_addr))
        .await
    ...
    let response = client.process_packet(request).await.expect("gRPC call failed");
